...
void setup() {
  ...
  // Initially, set the RED light ON and GREEN On
  digitalWrite(RED_LED_PIN, LOW);
  digitalWrite(GREEN_LED_PIN, LOW);
  digitalWrite(RED_LED_PIN, HIGH);
  digitalWrite(GREEN_LED_PIN, HIGH);
  delay(1500);
  Serial.println("Serial port success. Send 's' to attempt WiFi connection.");
  digitalWrite(GREEN_LED_PIN, LOW);
}
...
void loop() {
  ...
  const unsigned long connectionTimeout = 10000; // 15 seconds timeout

  if (Serial.available() > 0) {
    char receivedChar = Serial.read();
    if (receivedChar == 's' && !attemptingConnection) {
